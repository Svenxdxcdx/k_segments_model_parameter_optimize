...
# Helper methods
    ...
    def run_simulation(self, directory, training, test, monotonically_increasing = True, k = 4, collection_interval = 2):
        ...
        simulation = Simulation(self._simulations[self._currentModel + 2], directory, retry_mode = 'selective', provided_file_names = training)
        simulations.append(simulation)
        ...
        task_model = KSegmentsModel(k = k, monotonically_increasing = False)
        simulation = Simulation(task_model, directory, retry_mode = 'selective', provided_file_names = training)
        simulations.append(simulation)
...
# OUTPUT = ( [Waste: [Witt: 25, Tovar: 25, k-segments:25], [50] , [75]], [Retries], [Runtime])
    ...
    def runOneExperimentBoth(self):
        ...
        models = [self._fileNames[self._currentRun] + "_selective", self._fileNames[self._currentRun] + "_partial",self._fileNames[self._currentRun] + "_partial_notmonoton", "KSegmentsModel_selective", "KSegmentsModel_partial", "KSegmentsModel_partial_notmonoton"]  
        dictObject = {
            "models": models,
        ...
        if os.path.exists("pickleFiles\\"+ self.write_dir +"\\"+self._fileNames[self._currentRun] + ".pickle"):
            os.remove("pickleFiles\\"+ self.write_dir +"\\"+self._fileNames[self._currentRun] + ".pickle")
        with open( "pickleFiles\\"+ self.write_dir +"\\"+self._fileNames[self._currentRun] + ".pickle", "wb") as file:
            pickle.dump(dictObject, file, protocol=pickle.HIGHEST_PROTOCOL)
            
    def runOneExperimentSarek(self):
        self.write_dir = "sarek"
        base_directory = f'{BASE_DIR}/'+ self.write_dir 
        workflow_tasks = []
        file_order = self.get_file_order(base_directory)
        if file_order != None:
    ...
    def runOneExperimentEager(self):
        self.write_dir = "eager"
        base_directory = f'{BASE_DIR}/'+ self.write_dir 
        workflow_tasks = []
        file_order = self.get_file_order(base_directory)
        if file_order != None:
    ...
    def runAllExperiments(self):
        ...
        for i in range(self._maxRuns):
            ...
            self.runOneExperimentBoth()
            self.runOneExperimentSarek()
            self.runOneExperimentEager()
            self._currentModel = self._currentModel + 3
...
if __name__ == "__main__":
    ...
    models.append(ActiveFeedbackModel_k_segments(k = k, monotonically_increasing = monotonically_increasing))
    models.append(ActiveFeedbackModel_k_segments(k = k, monotonically_increasing = False))
